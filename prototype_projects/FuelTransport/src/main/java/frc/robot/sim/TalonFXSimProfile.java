...
class TalonFXSimProfile extends SimProfile {
  ...
  public TalonFXSimProfile(final TalonFX talonFX, final double rotorInertia) {
    ...
    this._motorSim = new DCMotorSim(
      LinearSystemId.createDCMotorSystem(gearbox, rotorInertia, 1.0),
      gearbox
    );
  ...
  public void run() {
    ...
    final double velocity_rps = Units.radiansToRotations(
      _motorSim.getAngularVelocityRadPerSec()
    );

    _talonFXSim.setRawRotorPosition(position_rot);
    _talonFXSim.setRotorVelocity(velocity_rps);

    _talonFXSim.setSupplyVoltage(
      12 - _talonFXSim.getSupplyCurrent() * kMotorResistance
    );
  }
}
