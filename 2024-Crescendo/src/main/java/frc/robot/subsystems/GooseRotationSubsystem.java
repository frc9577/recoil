...
public class GooseRotationSubsystem extends SubsystemBase {
  ...
  private final CANSparkMax m_rotationMotor = new CANSparkMax(
    GooseRotationConstants.kRotateMotorCANID,
  ...
  public GooseRotationSubsystem() {
    ...
    double angleDelta =
      GooseRotationConstants.kTopAngle - GooseRotationConstants.kBottomAngle;
    double encoderDelta =
      GooseRotationConstants.kTopRawEncoderValue -
      GooseRotationConstants.kBottomRawEncoderValue;
    m_encoderGradient = angleDelta / encoderDelta;
    m_encoderIntercept =
      GooseRotationConstants.kTopAngle -
      (m_encoderGradient * GooseRotationConstants.kTopRawEncoderValue);

    m_rotationMotor.restoreFactoryDefaults();
    m_rotationMotor.setInverted(true);
    m_rotationMotor.setSmartCurrentLimit(
      GooseRotationConstants.kMotorCurrentLimit
    );

    m_Encoder = m_rotationMotor.getAbsoluteEncoder(
    ...
    if (GooseRotationConstants.kUseSmartMotion) {
      m_pidController.setSmartMotionMaxVelocity(
        GooseRotationConstants.kMaxVelocity,
        GooseRotationConstants.kSmartMotionSlot
      );
      m_pidController.setSmartMotionMinOutputVelocity(
        GooseRotationConstants.kMinVelocity,
        GooseRotationConstants.kSmartMotionSlot
      );
      m_pidController.setSmartMotionMaxAccel(
        GooseRotationConstants.kMaxAcceleration,
        GooseRotationConstants.kSmartMotionSlot
      );
      m_pidController.setSmartMotionAllowedClosedLoopError(
        GooseRotationConstants.kMaxError,
        GooseRotationConstants.kSmartMotionSlot
      );
    }
    ...
    SmartDashboard.putNumber(
      "Set Reference",
      getMotorPositionFromAngle(m_angleSet)
    );
  }
