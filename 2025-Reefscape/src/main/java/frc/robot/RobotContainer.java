...
public class RobotContainer {
  ...
  private final SendableChooser<Command> m_autoChooser =
    new SendableChooser<>();
  private final SendableChooser<Boolean> m_isArcadeChooser =
    new SendableChooser<>();
  ...
  private final JoystickButton m_enableReverse = new JoystickButton(
    m_driverController,
    OperatorConstants.kEnableReverse
  );
  private final JoystickButton m_disableReverse = new JoystickButton(
    m_driverController,
    OperatorConstants.kDisableReverse
  );

  private final JoystickButton m_zeroButton = new JoystickButton(
    m_operatorController,
    OperatorConstants.kZeroElevator
  );
  private final JoystickButton m_increaseElevatorLevelButton =
    new JoystickButton(
      m_operatorController,
      OperatorConstants.kIncreaseElevatorLevel
    );
  private final JoystickButton m_decreeseElevatorlevelButton =
    new JoystickButton(
      m_operatorController,
      OperatorConstants.kDecreeseElevatorLevel
    );
  private final JoystickButton m_IntakeElevatorPositionButton =
    new JoystickButton(
      m_operatorController,
      OperatorConstants.kIntakeElevatorPosition
    );

  private final JoystickButton m_algaeIntakeButton = new JoystickButton(
    m_operatorController,
    OperatorConstants.kAlageIntake
  );
  private final JoystickButton m_algaeOutputButton = new JoystickButton(
    m_operatorController,
    OperatorConstants.kAlageOutput
  );

  private final JoystickButton m_coralIntakeButton = new JoystickButton(
    m_operatorController,
    OperatorConstants.kCoralIntake
  );
  private final JoystickButton m_coralOutputButton = new JoystickButton(
    m_operatorController,
    OperatorConstants.kCoralOuttake
  );
  private final JoystickButton m_coralStopMotorsButton = new JoystickButton(
    m_operatorController,
    OperatorConstants.kCoralStop
  );

  private final JoystickButton m_shooterButton = new JoystickButton(
    m_operatorController,
    OperatorConstants.kShooterShoot
  );
  ...
  private static <SSC> Optional<SSC> getSubsystem(Class<SSC> subsystemClass) {
    Optional<SSC> iss;
    try {
      iss = Optional.ofNullable(
        subsystemClass.getDeclaredConstructor().newInstance()
      );
    } catch (Exception e) {
      iss = Optional.empty();
  ...
  private void configureDriverStationControls() {
    ...
    m_autoChooser.setDefaultOption(
      "(PID) Drive forward 2 meters",
      new AutonomousDrivePID(m_driveSubsystem, 2.0, 2.0, 0.04)
    );
    m_autoChooser.addOption(
      "(PID) Drive forward 5 meters",
      new AutonomousDrivePID(m_driveSubsystem, 5.0, 5.0, 0.04)
    );
    m_autoChooser.addOption(
      "(PID) Drive backward 2 meters",
      new AutonomousDrivePID(m_driveSubsystem, -2.0, -2.0, 0.04)
    );
    m_autoChooser.addOption(
      "(PID) Drive backward 5 meters",
      new AutonomousDrivePID(m_driveSubsystem, -5.0, -5.0, 0.04)
    );

    m_autoChooser.addOption(
      "(PID) Drive forward 2 meters then backward 2 meters",
      new AutonomousDrivePID(m_driveSubsystem, 2.0, 2.0, 0.04)
    ...
    m_autoChooser.addOption(
      "(Gyro) Turn 90 Degrees counter-clockwise",
      new AutonomousTurnDegrees(m_driveSubsystem, -90, 0.3)
    );

    m_autoChooser.addOption(
      "(Gyro) Turn 90 degrees clockwise then 90 degres counter-clockwise",
      new AutonomousTurnDegrees(m_driveSubsystem, 90, 0.3)
        .andThen(new TimedCommand(new EmptyCommand(), 1000))
    ...
    m_autoChooser.addOption(
      "(Combined) Drive from blue most right to reef",
  ...
  private void configureBindings() {
    ...
    SmartDashboard.putBoolean(
      "Intake Subsystem",
      m_intakeSubsystem.isPresent()
    );
    SmartDashboard.putBoolean("Coral Subsystem", m_coralSubsystem.isPresent());
    SmartDashboard.putBoolean(
      "Shooter Subsystem",
      m_shooterSubsystem.isPresent()
    );
    SmartDashboard.putBoolean(
      "Elevator Subsystem",
      m_elevatorSubsystem.isPresent()
    );
  ...
  public void UpdateSmartDashboard() {
    ...
    if ((m_iTickCount % Constants.DrivetrainConstants.kTicksPerUpdate) == 0) {
      SmartDashboard.putNumber(
        "Left Encoder",
        m_driveSubsystem.getPosition(true)
      );
      SmartDashboard.putNumber(
        "Left Target",
        m_driveSubsystem.getTargetPosition(true)
      );
      SmartDashboard.putNumber("Left Speed", m_driveSubsystem.getSpeed(true));

      SmartDashboard.putNumber(
        "Right Encoder",
        m_driveSubsystem.getPosition(false)
      );
      SmartDashboard.putNumber(
        "Right Target",
        m_driveSubsystem.getTargetPosition(false)
      );
    ...
    if (
      m_coralSubsystem.isPresent() &&
      (m_iTickCount % Constants.CoralConstants.kTicksPerUpdate) == 0
    ) {
      SmartDashboard.putBoolean("Has Coral", m_coralSubsystem.get().hasCoral());
      SmartDashboard.putNumber(
        "Input Speed",
        m_coralSubsystem.get().getIntakeSpeed()
      );
      SmartDashboard.putNumber(
        "Output Speed",
        m_coralSubsystem.get().getOutputSpeed()
      );
    }
    ...
    if (
      m_elevatorSubsystem.isPresent() &&
      (m_iTickCount % Constants.ElevatorConstants.kTicksPerUpdate) == 0
    ) {
      ElevatorSubsystem elevatorSubsystem = m_elevatorSubsystem.get();
      ...
      SmartDashboard.putNumber(
        "Elevator Set Point",
        elevatorSubsystem.getTargetPosition()
      );
      SmartDashboard.putNumber(
        "Elevator Speed",
        elevatorSubsystem.getMotorSpeed()
      );
      SmartDashboard.putNumber(
        "Elevator Level Index",
        elevatorSubsystem.getElevatorLevelIndex()
      );

      SmartDashboard.putBoolean(
        "Elevator Down",
        elevatorSubsystem.isElevatorDown()
      );
    }
    ...
    if (
      m_intakeSubsystem.isPresent() &&
      (m_iTickCount % Constants.IntakeConstants.kTicksPerUpdate) == 0
    ) {
    ...
    // Compressor
    if (
      (m_iTickCount % Constants.RobotConstants.kpnuematicsTicksPerUpdate) == 0
    ) {
    ...
    if (
      m_algaeSubsystem.isPresent() &&
      (m_iTickCount % Constants.AlgaeConstants.kTicksPerUpdate) == 0
    ) {
