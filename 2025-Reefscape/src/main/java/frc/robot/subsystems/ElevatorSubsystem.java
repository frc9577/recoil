...
public class ElevatorSubsystem extends SubsystemBase {
  ...
  private final DigitalInput m_LineBreakSensor = new DigitalInput(
    ElevatorConstants.kSensorChannel
  );
  private boolean m_sensorBroken = false;
  private final PositionVoltage m_positionVoltage = new PositionVoltage(
    0
  ).withSlot(0);
  ...
  public ElevatorSubsystem() {
    ...
    configs.Voltage.withPeakForwardVoltage(
      Volts.of(ElevatorConstants.PeakVoltage)
  ...
  public void setTargetPosition(double height) {
    ...
    double elevatorPositionRotations =
      height / ElevatorConstants.kElevatorGearRatio;
    if (!ElevatorConstants.kPositiveMovesUp) {
      elevatorPositionRotations = -elevatorPositionRotations;
    }

    m_motor.setControl(
      m_positionVoltage.withPosition(elevatorPositionRotations)
    );
  }
